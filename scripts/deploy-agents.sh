#!/bin/bash
# Kolibri Mesh — Deploy agents using SSH config aliases
# Uses ~/.ssh/config for all connections

set -e

COORDINATOR_URL="http://178.207.11.90:8080"
AGENT_BIN="/Users/kolibri/kolibri-mesh/bin/agent-linux"

# Colors
GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
NC='\033[0m'

echo -e "${GREEN}Kolibri Mesh — Deploying agents${NC}"
echo "Coordinator: $COORDINATOR_URL"
echo ""

# Deploy function using SSH config alias
deploy() {
  local host=$1
  local node_id=$2
  local node_ip=$3
  
  echo -n "  $host ($node_ip)... "
  
  # Copy binary
  scp -o ConnectTimeout=10 -q "$AGENT_BIN" "$host:/tmp/kolibri-mesh-agent" 2>/dev/null
  
  # Create start script and run
  ssh -o ConnectTimeout=10 "$host" bash -s << REMOTE_SCRIPT
    chmod +x /tmp/kolibri-mesh-agent
    pkill -9 -f kolibri-mesh-agent 2>/dev/null || true
    sleep 1
    cat > /tmp/start-kolibri-mesh.sh << 'EOF'
#!/bin/bash
export NODE_ID="$node_id"
export NODE_NAME="$node_id"
export COORDINATOR_URL="$COORDINATOR_URL"
export NODE_IP="$node_ip"
exec /tmp/kolibri-mesh-agent
EOF
    chmod +x /tmp/start-kolibri-mesh.sh
    nohup /tmp/start-kolibri-mesh.sh > /tmp/kolibri-mesh-agent.log 2>&1 &
REMOTE_SCRIPT

  if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅${NC}"
  else
    echo -e "${RED}❌${NC}"
  fi
}

# MikroTik proxy — координатор через Home
echo -e "${YELLOW}=== Deploying via SSH config ===${NC}"
echo ""

# VPN servers (через home → main)
deploy "kolibri-main"        "main"        "10.99.0.2"
deploy "kolibri-uiap"        "uiap"        "10.99.0.3"
deploy "kolibri-qjns"        "qjns"        "10.99.0.4"
deploy "kolibri-9fts"        "9fts"        "10.99.0.5"
deploy "kolibri-new"         "new"         "10.99.0.6"

# Direct servers
deploy "kolibri-direct"      "direct"      "78.17.4.108"
deploy "kolibri-worker-backup" "worker-backup" "109.248.161.39"

# Hostvds servers
deploy "reserve242"          "reserve242"     "31.57.26.242"
deploy "hostvds-highload"    "highload"       "45.38.139.182"
deploy "hostvds-agent-01"    "agent-01"       "31.57.27.128"
deploy "hostvds-agent-02"    "agent-02"       "213.232.204.223"
deploy "hostvds-agent-03"    "agent-03"       "188.130.206.204"
deploy "hostvds-agent-04"    "agent-04"       "31.59.41.146"
deploy "hostvds-agent-05"    "agent-05"       "31.56.196.10"
deploy "hostvds-agent-06"    "agent-06"       "45.39.33.252"
deploy "hostvds-agent-07"    "agent-07"       "46.8.225.34"
deploy "hostvds-agent-08"    "agent-08"       "31.59.105.200"
deploy "hostvds-agent-09"    "agent-09"       "95.182.84.254"
deploy "hostvds-agent-10"    "agent-10"       "217.60.38.191"
deploy "hostvds-paris-highload" "paris"        "95.182.83.60"

# Additional servers
deploy "kolibri-inference-recovery" "inference" "94.183.235.154"
deploy "kolibri-main-api-direct"    "main-api"  "104.253.43.117"
deploy "kolibri-rag-knowledge-direct" "rag"      "31.57.26.151"
deploy "kolibri-tools-executor-direct" "tools"   "217.60.63.97"

echo ""
echo -e "${GREEN}=== Done! ===${NC}"
echo "Check status: curl $COORDINATOR_URL/api/nodes"
